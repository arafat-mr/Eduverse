...
function UserCertificatesPage() {
  const { user, authLoading } = useAuth(); // Destructure correctly
  const [certificates, setCertificates] = useState([]);
  const [loading, setLoading] = useState(false); // Data fetch loading

  useEffect(() => {
    if (!user?.email || authLoading) return; // Only fetch when user exists and auth is ready
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => {
            ...
            fetch(`/api/userCertificates?email=${user.email}`)
              .then((res) => res.json())
